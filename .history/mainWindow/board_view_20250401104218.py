...
class BoardWidget(QWidget):
    ...
    def __init__(self, parent=None, style_index=0):
        ...
        self.main_layout = QHBoxLayout(self)ding, QSizePolicy.Expanding)
        self.main_layout.setContentsMargins(20, 20, 20, 20)
        self.main_layout.setSpacing(20)  # 增加间距
        self.main_layout = QHBoxLayout(self)
        # 创建左侧容器，用于保持棋盘居中setContentsMargins(20, 20, 20, 20)
        self.left_container = QWidget()  # 增加间距
        self.left_layout = QVBoxLayout(self.left_container)
        self.left_layout.setContentsMargins(0, 0, 0, 0)
        self.left_container = QWidget()
        # 创建棋盘实例t_layout = QVBoxLayout(self.left_container)
        self.board = GoBoardWidget(self, style_index)0)
        
        # 左侧容器采用扩展策略，允许随窗口放大
        self.left_container.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        
        # 将棋盘添加到左侧容器，使用Qt.AlignCenter但允许扩展
        self.left_layout.addWidget(self.board, 1, Qt.AlignCenter)QSizePolicy.Expanding)
        
        # 创建右侧控制面板容器，完全填充
        self.right_panel = QWidget()elf.board, 1)  # 移除了Qt.AlignCenter参数，让棋盘完全填充容器
        self.right_layout = QVBoxLayout(self.right_panel)
        self.right_layout.setContentsMargins(10, 10, 10, 10)
        self.right_layout.setSpacing(15)
        self.right_layout = QVBoxLayout(self.right_panel)
        # 设置右侧面板的固定宽度，防止过度拉伸tContentsMargins(10, 10, 10, 10)
        self.right_panel.setFixedWidth(300)
        
        # 创建标题标签的宽度策略，使其不随窗口拉伸
        self.title_label = QLabel("五子棋游戏")zePolicy.Fixed, QSizePolicy.Preferred)
        self.title_label.setAlignment(Qt.AlignCenter)
        title_font = self.title_label.font()
        title_font.setPointSize(24)
        title_font.setBold(True)l("五子棋游戏")
        self.title_label.setFont(title_font)gnCenter)
        title_font = self.title_label.font()
        # 创建操作区域布局.setPointSize(24)
        self.controls_layout = QHBoxLayout()
        self.title_label.setFont(title_font)
        # 创建棋盘风格选择下拉框
        self.style_label = QLabel("棋盘风格：")
        self.style_combo = ComboBox(self)t()
        ...
        # 填充风格选择下拉框
        # 将标签和下拉框添加到操作区域.addItems(self.board.get_style_names())
        self.controls_layout.addWidget(self.style_label)
        self.controls_layout.addWidget(self.style_combo)f.change_board_style)
        self.controls_layout.addStretch(1)
        # 将标签和下拉框添加到操作区域
        # 创建分隔线ntrols_layout.addWidget(self.style_label)
        self.separator = QFrame()idget(self.style_combo)
        self.separator.setFrameShape(QFrame.HLine)
        self.separator.setFrameShadow(QFrame.Sunken)
        # 创建分隔线
        # 玩家信息标签arator = QFrame()
        self.player_info = QLabel("当前玩家：黑棋")HLine)
        ...
        self.player_info.setAlignment(Qt.AlignCenter)
        # 创建游戏说明t = self.player_info.font()
        self.game_instructions = QLabel(
            "游戏说明：\n"nfo.setFont(info_font)
            "1. 点击「开始对局」按钮开始游戏\n"
            "2. 黑棋先行，双方轮流下棋\n"
            "3. 先连成五子一线者获胜\n"s = QLabel(
            "4. 点击「重新开始」可随时重置游戏"
        ...
        self.start_button = PushButton("开始对局").AlignLeft)
        self.restart_button = PushButton("重新开始")
        # 创建按钮布局
        # 设置按钮尺寸ton_layout = QVBoxLayout()  # 改为垂直布局
        self.start_button.setFixedHeight(40)")
        self.restart_button.setFixedHeight(40)")
        
        # 按钮添加到布局
        self.button_layout.addWidget(self.start_button)
        self.button_layout.addSpacing(10)t(40)
        self.button_layout.addWidget(self.restart_button)
        # 按钮添加到布局
        # 添加组件到右侧布局_layout.addWidget(self.start_button)
        self.right_layout.addWidget(self.title_label)
        self.right_layout.addSpacing(10)f.restart_button)
        ...
        self.right_layout.addSpacing(20).separator)
        self.right_layout.addLayout(self.button_layout)
        self.right_layout.addStretch(1)  # 添加伸缩器使组件靠上对齐
        self.right_layout.addWidget(self.game_instructions)
        # 将棋盘容器和右侧面板添加到主布局，并添加两侧伸缩项g(20)
        self.main_layout.addStretch(1)lf.button_layout)
...
class BoardWindow(FramelessWindow):
    """五子棋游戏窗口"""
    s BoardWindow(FramelessWindow):
    def __init__(self, parent=None, style_index=0):
        super().__init__(parent)
        __init__(self, parent=None, style_index=0):
        # 设置窗口标题和图标nit__(parent)
        self.setWindowTitle("五子棋游戏")
        self.setWindowIcon(FIF.GAME.icon())
        self.setWindowTitle("五子棋游戏")
        # 设置窗口为独立窗口dowIcon(FIF.GAME.icon())
        self.setWindowFlags(Qt.Window)
        # 设置窗口为独立窗口
        # 设置窗口大小，增加宽度以适应新布局(Qt.Window)
        self.resize(900, 700)
        # 设置窗口大小，增加宽度以适应新布局
        # 设置窗口在屏幕中央显示00, 700)
        screen = QApplication.desktop().availableGeometry()
        self.move(央显示
            (screen.width() - self.width()) // 2,Geometry()
        ...
        # 创建主窗口部件
        # 创建主布局并设置更宽松的边距ntral_widget = QWidget(self)
        self.main_layout = QVBoxLayout(self.central_widget)
        self.main_layout.setContentsMargins(10, 10, 10, 10)  # 减小边距，为棋盘留更多空间
        self.main_layout = QVBoxLayout(self.central_widget)
        # 创建棋盘组件并设置尺寸策略n_layout.setContentsMargins(20, 20, 20, 20)
        self.board_widget = BoardWidget(self, style_index)
        self.board_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)# 创建棋盘组件
        ...
        self.close_button.clicked.connect(self.close)150)  # 设置固定宽度
        self.button_layout.addStretch(1)
        self.button_layout.addWidget(self.close_button)
        self.button_layout.addStretch(1)self.button_layout.addWidget(self.close_button)
        n_layout.addStretch(1)
        # 添加到布局时设置拉伸因子
        self.main_layout.addWidget(self.board_widget, 1)  # 添加拉伸因子，确保棋盘占据所有可用空间
        self.main_layout.addLayout(self.button_layout, 0)  # 按钮布局不拉伸self.main_layout.addWidget(self.board_widget)
        ayout.addLayout(self.button_layout)
        # 添加组件到主布局
        self.main_layout.addWidget(self.board_widget)
        self.main_layout.addLayout(self.button_layout)
            self.layout().setContentsMargins(0, 48, 0, 0)  # 为标题栏留出空间
        # 设置窗口的内容部件self.central_widget)
        self.setLayout(QVBoxLayout(self))
        self.layout().setContentsMargins(0, 48, 0, 0)  # 为标题栏留出空间
        self.layout().addWidget(self.central_widget)
    
    def closeEvent(self, event):        # 这里可以添加任何需要的资源清理代码
        """窗口关闭时的清理工作"""        super().closeEvent(event)
        print("游戏窗口正在关闭，清理资源...")
        # 这里可以添加任何需要的资源清理代码
...
if __name__ == "__main__":)
    ...
    sys.exit(app.exec_())    window.show()    window = BoardWindow()    app = QApplication(sys.argv)    sys.exit(app.exec_())
