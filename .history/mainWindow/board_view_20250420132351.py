...
class BoardWidget(QWidget):
    ...
    def __init__(self, parent=None, style_index=0):
        ...
        # 创建执棋方选择下拉框
        self.player_label = QLabel("执棋方：")
        self.player_combo = ComboBox(self)
        self.player_combo.addItems(["黑棋", "白棋"])
        self.player_combo.setCurrentIndex(0)
        self.player_combo.currentIndexChanged.connect(self.change_player_side)
        
        # 将标签和下拉框添加到操作区域
        self.controls_layout.addWidget(self.style_label)
        self.controls_layout.addWidget(self.style_combo)
        self.controls_layout.addWidget(self.player_label)
        self.controls_layout.addWidget(self.player_combo)
    ...
    def change_player_side(self, index):
        """更改执棋方"""
