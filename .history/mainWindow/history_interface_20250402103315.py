...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, QListWidget, 
                             ...
                             QComboBox, QGridLayout, QCheckBox, QScrollArea)
...
from qfluentwidgets import (PushButton, SearchLineEdit, InfoBar, InfoBarPosition, 
                           FluentIcon as FIF, isDarkTheme, FastCalendarPicker, FlyoutAnimationType)
...
class HistoryInterface(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 日期筛选 - 使用FastCalendarPicker替换QDateEdit
        self.date_label = QLabel("日期筛选:")
        
        # 创建从日期选择器
        self.date_from_label = QLabel("从:")
        self.date_from_picker = FastCalendarPicker(self)
        self.date_from_picker.setDateFormat('yyyy-MM-dd')
        self.date_from_picker.setDate(QDate.currentDate().addMonths(-1))  # 默认显示一个月前
        self.date_from_picker.dateChanged.connect(self.filter_history)
        
        # 创建至日期选择器
        self.date_to_label = QLabel("至:")
        self.date_to_picker = FastCalendarPicker(self)
        self.date_to_picker.setDateFormat('yyyy-MM-dd')
        self.date_to_picker.setDate(QDate.currentDate())  # 默认显示今天
        self.date_to_picker.dateChanged.connect(self.filter_history)
        
        # 设置日期选择器的飞出动画
        self.date_from_picker.setFlyoutAnimationType(FlyoutAnimationType.FADE_IN)
        self.date_to_picker.setFlyoutAnimationType(FlyoutAnimationType.FADE_IN)
        ...
        self.search_filter_layout.addWidget(self.date_from_label, 1, 1)
        self.search_filter_layout.addWidget(self.date_from_picker, 1, 2)
        self.search_filter_layout.addWidget(self.date_to_label, 2, 1)
        self.search_filter_layout.addWidget(self.date_to_picker, 2, 2)
        self.search_filter_layout.addWidget(self.date_filter_checkbox, 3, 0, 1, 2)
        self.search_filter_layout.addWidget(self.favorites_only_checkbox, 3, 2, 1, 2)
    ...
    def filter_history(self):
        ...
        # 获取日期范围 - 使用FastCalendarPicker
        from_date = self.date_from_picker.date().toPyDate()
        to_date = self.date_to_picker.date().toPyDate()
    ...
    def updateStyle(self):
        ...
        if isDarkTheme():
            ...
            self.history_list.setStyleSheet("background-color: #2d2d2d; color: white;")
        else:
