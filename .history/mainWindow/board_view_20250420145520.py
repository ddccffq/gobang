...
class BoardWidget(QWidget):
    ...
    def __init__(self, parent=None, style_index=0):
        ...
        # 添加组件到右侧布局，注意将原先的controls_layout替换为两个单独的布局
        self.right_layout.addWidget(self.title_label)
        self.right_layout.addSpacing(10)
        self.right_layout.addLayout(self.style_layout)  # 先添加棋盘风格布局
        self.right_layout.addLayout(self.side_layout)   # 再添加执棋方布局
    ...
    def onEndGame(self):
        ...
        InfoBar.info(
            title='游戏已结束',
            content="游戏已结束，棋盘已清空，请点击「开始对局」按钮开始新游戏",
            orient=Qt.Horizontal,
            isClosable=True,
            position=InfoBarPosition.TOP,
            duration=3000,
    ...
    def update_player_info(self):
        ...
        if self.board.game_over:
            ...
            player_text = f"当前玩家：黑棋{'(您)' if self.player_side == 'black' else '(AI)'}" if self.board.current_player == 1 else f"当前玩家：白棋{'(您)' if self.player_side == 'white' else '(AI)'}"
        self.player_info.setText(player_text)
        
        # 更新是否为人类玩家回合
        curr_side = "black" if self.board.current_player == 1 else "white"
        self.is_human_turn = (curr_side == self.player_side)
