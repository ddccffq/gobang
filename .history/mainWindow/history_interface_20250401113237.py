...
class HistoryListItem(QWidget):
    ...
    def __init__(self, title, date, winner, parent=None):
        ...
        # 创建胜者标签
        self.winner_label = QLabel(f"胜者: {winner}" if winner else "未结束")
        ...
        try:
            ...
            for file in files:
                try:
                    ...
                    item.setSizeHint(widget.sizeHint())
                    item.setData(Qt.UserRole, os.path.join(self.history_dir, file))  # 保存文件路径
                    
                    self.history_list.addItem(item)
                    self.history_list.setItemWidget(item, widget)
                    
                except Exception as e:
                    print(f"加载记录文件失败: {file}, 错误: {str(e)}")
