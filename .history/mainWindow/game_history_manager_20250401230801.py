...
class GameHistoryManager:
    ...
    def load_settings(self):
        """加载设置"""
        default_history_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "game_history")
        ...
        if os.path.exists(self.config_file):
            try:
                ...
                self.history_dir = config.get('history_dir', default_history_dir)
    ...
    def get_history_list(self):
        ...
        # 按修改时间倒序排序，确保最新的记录在前面
        history_list.sort(key=lambda x: x.get('modified_time', 0), reverse=True)
        return history_list
