...
class GameHistoryManager:
    ...
    def get_history_list(self):
        ...
        # 获取所有json文件
        try:
            files = [f for f in os.listdir(self.history_dir) if f.lower().endswith('.json')]
            
            # 如果没有文件，尝试扫描子目录
            if not files:
                for root, dirs, files_in_dir in os.walk(self.history_dir):
                    for file in files_in_dir:
                        if file.lower().endswith('.json'):
                            full_path = os.path.join(root, file)
                            rel_path = os.path.relpath(full_path, self.history_dir)
                            files.append(rel_path)
        except Exception as e:
            print(f"扫描历史记录目录失败: {str(e)}")
            return history_list
