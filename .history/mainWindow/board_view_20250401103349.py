...
class GoBoardWidget(QWidget):
    ...
    def __init__(self, parent=None, style_index=0):
        ...
        self.cell_size = 40   # 增加每个格子的大小，从30增加到40
        self.padding = 25     # 增加边距，从20增加到25
        self.stone_size = 36  # 增加棋子大小，从28增加到36
    ...
    def paintEvent(self, event: QPaintEvent):
        ...
        for x, y in star_points:
            ...
            painter.drawEllipse(
                self.padding + x * self.cell_size - 4,  # 增大星位标记
                self.padding + y * self.cell_size - 4,
                8, 8
            )
        ...
        if not self.game_started:
            ...
            # 创建半透明背景 - 修复：将浮点数转换为整数
            rect = QRect(
                int(self.width() / 4),        # 将浮点数转换为整数
                int(self.height() / 2 - 20),  # 将浮点数转换为整数
                int(self.width() / 2),        # 将浮点数转换为整数
                40
            )
...
class BoardWidget(QWidget):
    ...
    def __init__(self, parent=None, style_index=0):
        ...
        # 创建右侧控制面板layout.setContentsMargins(0, 0, 0, 0)
        self.right_panel = QWidget()
        self.right_layout = QVBoxLayout(self.right_panel)
        self.right_layout.setContentsMargins(10, 10, 10, 10)
        self.right_layout.setSpacing(15)
        # 将棋盘添加到左侧容器并居中
        # 创建标题标签t_layout.addWidget(self.board, 0, Qt.AlignCenter)
        self.title_label = QLabel("五子棋游戏")
        self.title_label.setAlignment(Qt.AlignCenter)
        title_font = self.title_label.font()
        title_font.setPointSize(24)yout(self.right_panel)
        title_font.setBold(True)tentsMargins(10, 10, 10, 10)
        ...
        self.style_combo = ComboBox(self)AlignCenter)
        title_font = self.title_label.font()
        # 填充风格选择下拉框setPointSize(24)
        ...
        # 创建操作区域布局
        # 将标签和下拉框添加到操作区域yout = QHBoxLayout()
        ...
        self.separator.setFrameShape(QFrame.HLine)tyle_names())
        self.separator.setFrameShadow(QFrame.Sunken))
        self.style_combo.currentIndexChanged.connect(self.change_board_style)
        # 玩家信息标签
        self.player_info = QLabel("当前玩家：黑棋")
        self.player_info.setAlignment(Qt.AlignCenter)el)
        info_font = self.player_info.font().style_combo)
        info_font.setPointSize(16)retch(1)
        self.player_info.setFont(info_font)
        # 创建分隔线
        # 创建游戏说明arator = QFrame()
        self.game_instructions = QLabel(ame.HLine)
            "游戏说明：\n"r.setFrameShadow(QFrame.Sunken)
            "1. 点击「开始对局」按钮开始游戏\n"
            "2. 黑棋先行，双方轮流下棋\n"
            "3. 先连成五子一线者获胜\n"abel("当前玩家：黑棋")
            "4. 点击「重新开始」可随时重置游戏"nment(Qt.AlignCenter)
        )nfo_font = self.player_info.font()
        ...
        self.right_layout.addSpacing(20))
        self.right_layout.addLayout(self.button_layout)n)
        self.right_layout.addStretch(1)  # 添加伸缩器使组件靠上对齐
        # 添加组件到右侧布局
        # 将棋盘和右侧面板添加到主布局t.addWidget(self.title_label)
        self.main_layout.addWidget(self.board)
        self.main_layout.addWidget(self.right_panel)yout)
        self.right_layout.addWidget(self.separator)
        # 设置对象名称，用于导航yout.addWidget(self.player_info)
        self.setObjectName('App-Interface')
        self.right_layout.addWidget(self.game_instructions)
        # 连接按钮事件ht_layout.addSpacing(20)
        self.start_button.clicked.connect(self.onStartGame)
        self.restart_button.clicked.connect(self.onRestartGame)
        
        # 初始化时更新玩家信息添加到主布局，并添加两侧伸缩项
        self.update_player_info()ch(1)
        self.main_layout.addWidget(self.left_container)
        # 设置初始游戏状态layout.addWidget(self.right_panel)
        self.board.game_started = False
        
    def change_board_style(self, index):
        """更改棋盘风格"""ctName('App-Interface')
        self.board.set_style(index)
        # 连接按钮事件
    def update_player_info(self):.connect(self.onStartGame)
        """更新当前玩家信息"""utton.clicked.connect(self.onRestartGame)
    ...
    def onStartGame(self):
        ...
        self.update_player_info()
        change_board_style(self, index):
        # 显示提示信息格"""
        InfoBar.success(tyle(index)
            title='游戏已开始',
            content="黑棋先行，请点击棋盘落子",
            orient=Qt.Horizontal,
            isClosable=True,黑棋" if self.board.current_player == 1 else "当前玩家：白棋"
            position=InfoBarPosition.TOP,ext)
            duration=3000,
            parent=selff):
        )""开始游戏"""
        self.board.reset_game()  # 这个方法内部会设置game_started为True
    def onRestartGame(self):nfo()
        """重新开始游戏"""
        self.board.reset_game()
        self.update_player_info()
            title='游戏已开始',
        # 显示提示信息ent="黑棋先行，请点击棋盘落子",
        InfoBar.info(.Horizontal,
            title='游戏已重置',e,
            content="棋盘已清空，黑棋先行",ion.TOP,
            orient=Qt.Horizontal,
            isClosable=True,
...
class BoardWindow(FramelessWindow):
    ...
    def __init__(self, parent=None, style_index=0):
        super().__init__(parent),
            orient=Qt.Horizontal,
        # 设置窗口标题和图标ble=True,
        self.setWindowTitle("五子棋游戏").TOP,
        self.setWindowIcon(FIF.GAME.icon())
            parent=self
        # 设置窗口为独立窗口
        self.setWindowFlags(Qt.Window)
        
        # 设置窗口大小，增加宽度以适应新布局Window):
        self.resize(900, 700)
        
        # 设置窗口在屏幕中央显示, parent=None, style_index=0):
        screen = QApplication.desktop().availableGeometry()
        self.move(
            (screen.width() - self.width()) // 2,
            (screen.height() - self.height()) // 2
        )elf.setWindowIcon(FIF.GAME.icon())
        ...
        self.main_layout.setContentsMargins(20, 20, 20, 20)
        # 设置窗口在屏幕中央显示
        # 创建棋盘组件 QApplication.desktop().availableGeometry()
        self.board_widget = BoardWidget(self, style_index)
            (screen.width() - self.width()) // 2,
        # 创建关闭按钮布局，并添加伸缩项让内容居中een.height() - self.height()) // 2
        self.button_layout = QHBoxLayout()
        self.close_button = PushButton("关闭窗口")
        self.close_button.setFixedWidth(150)  # 设置固定宽度
        self.close_button.clicked.connect(self.close)lf)
        self.button_layout.addStretch(1)
        self.button_layout.addWidget(self.close_button)
        self.button_layout.addStretch(1)self.main_layout = QVBoxLayout(self.central_widget)
        layout.setContentsMargins(20, 20, 20, 20)
        # 添加组件到主布局
        self.main_layout.addWidget(self.board_widget)
        self.main_layout.addLayout(self.button_layout)self.board_widget = BoardWidget(self, style_index)
        
        # 设置窗口的内容部件
        self.setLayout(QVBoxLayout(self))
        self.layout().setContentsMargins(0, 48, 0, 0)  # 为标题栏留出空间
        self.layout().addWidget(self.central_widget)    self.close_button.clicked.connect(self.close)
    retch(1)
    def closeEvent(self, event):ut.addWidget(self.close_button)
        """窗口关闭时的清理工作"""etch(1)
        print("游戏窗口正在关闭，清理资源...")
        # 这里可以添加任何需要的资源清理代码
        super().closeEvent(event)        self.main_layout.addWidget(self.board_widget)
        self.main_layout.addLayout(self.button_layout)

if __name__ == "__main__":设置窗口的内容部件
    # 测试代码ut(self))
    app = QApplication(sys.argv)ontentsMargins(0, 48, 0, 0)  # 为标题栏留出空间
    window = BoardWindow()ut().addWidget(self.central_widget)
    window.show()
    sys.exit(app.exec_())    def closeEvent(self, event):
